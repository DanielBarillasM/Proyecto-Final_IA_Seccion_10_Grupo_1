...
\begin{enumerate}[leftmargin=2em,itemsep=4pt]
  \item \textbf{M\'odulo ML}: validaci\'on cruzada 5-fold sobre datos hist\'oricos
    sint\'eticos generados con distribuciones realistas; comparaci\'on de 5~modelos.
  \item \textbf{M\'odulo CSP}: instancias de tama\~no Tiny/Small/Medium/Large;
    comparaci\'on de BT~puro, BT+FC+MRV+LCV y Beam~Search.
  \item \textbf{M\'odulo Bayes}: validaci\'on de la red bayesiana con tablas de
    probabilidad condicional estimadas; comparaci\'on con frecuencias emp\'iricas.
  \item \textbf{M\'odulo MDP}: evaluaci\'on de la pol\'itica mediante simulaci\'on
    de contingencias aleatorias; comparaci\'on con pol\'itica aleatoria y greedy.
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em,itemsep=6pt]
  ...
  \item \textbf{Superiodidad esperada del enfoque h\'ibrido.}
    Con base en los fundamentos te\'oricos, se espera que el sistema completo
    (Tabla~\ref{tab:ablation}) supere a cualquier m\'odulo en solitario,
    ya que la predicci\'on de ML reduce el dominio del CSP, la red bayesiana
    detecta riesgos que el CSP no modela, y el MDP garantiza resiliencia ante
    contingencias.
